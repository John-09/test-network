...
function printHelp () {
  echo "Usage: "
  echo "  ${ORG_NAME}.sh up|down|generate [-c <channel name>] [-t <timeout>] [-d <delay>] [-f <docker-compose-file>] [-s <dbtype>]"
  echo "  ${ORG_NAME}.sh -h|--help (print this message)"
  echo "    <mode> - one of 'up', 'down', or 'generate'"
  echo "      - 'up' - add ${ORG_NAME} to the sample network. You need to bring up the test network and create a channel first."
  echo "      - 'down' - bring down the test network and ${ORG_NAME} nodes"
  ...
  echo
  echo "	${ORG_NAME}.sh generate"
  echo "	${ORG_NAME}.sh up"
  echo "	${ORG_NAME}.sh up -c mychannel -s couchdb"
  echo "	${ORG_NAME}.sh down"
  echo
...
function generateOrg() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    echo ${ORG_NAME}
    sed -e "s/\${ORG_NAME}/${ORG_NAME}/" org-crypto.yaml > org-crypto-${ORG_NAME}.yaml
    set -x
    cryptogen generate --config="org-crypto-${ORG_NAME}.yaml" --output="../organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
...
function generateOrgDefinition() {
  ...
  export FABRIC_CFG_PATH=$PWD
  set -x
  # sed -e "s/\${ORG_NAME}/${ORG_NAME}/" configtx.yaml > configtx-${ORG_NAME}.yaml
  configtxgen -printOrg "${ORG_NAME}MSP" > ../organizations/peerOrganizations/"${ORG_NAME}.example.com"/"${ORG_NAME}.json"
  res=$?
  { set +x; } 2>/dev/null
  if [ $res -ne 0 ]; then
...
function addOrg () {
  ...
  if [ ! -d "../organizations/peerOrganizations/${ORG_NAME}.example.com" ]; then
    generateOrg
  ...
  PWD
  # cd ../scripts/org-scripts
  # pwd 
  # docker exec cli ./scripts/org-scripts/updateChannelConfig.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE
  ${CONTAINER_CLI} exec cli ./scripts/org-scripts/updateChannelConfig.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE
  # cat ../scripts/org-scripts/updateChannelConfig.sh
  # cat updateChannelConfig.sh
  # cd ../../addOrg3
  pwd
  if [ $? -ne 0 ]; then
  ...
  ${CONTAINER_CLI} exec cli ./scripts/org-scripts/joinChannel.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
...
COMPOSE_FILE_COUCH_BASE=compose/compose-couch-org.yaml
COMPOSE_FILE_COUCH_ORG=compose/${CONTAINER_CLI}/docker-compose-couch-org.yaml
...
COMPOSE_FILE_CA_BASE=compose/compose-ca-org.yaml
COMPOSE_FILE_CA_ORG=compose/${CONTAINER_CLI}/docker-compose-ca-org.yaml
# database
DATABASE="leveldb"
